...
public interface DyeableTackItem {
    String TAG_COLOR = "color";
    String TAG_DISPLAY = "display";
    int DEFAULT_TACK_COLOR = -1;

    default int getDefaultTackColor() {return -1;}
    ...
    default int getColor(ItemStack pStack) {
        CompoundTag compoundtag = pStack.getTagElement("display");
        return compoundtag != null && compoundtag.contains("color", 99) ? compoundtag.getInt("color") : this.getDefaultTackColor();
    }
    ...
    static ItemStack dyeItem(ItemStack pStack, List<DyeItem> pDyes) {
        ItemStack itemstack = ItemStack.EMPTY;
        int[] aint = new int[3];
        int i = 0;
        int j = 0;
        DyeableTackItem dyeableTackItem = null;
        Item item = pStack.getItem();
        if (item instanceof DyeableTackItem) {
            dyeableTackItem = (DyeableTackItem)item;
            itemstack = pStack.copy();
            itemstack.setCount(1);
            if (dyeableTackItem.hasCustomColor(pStack)) {
                int k = dyeableTackItem.getColor(itemstack);
        ...
        if (dyeableTackItem == null) {
            ...
            int j2 = (j1 << 8) + k1;
            j2 = (j2 << 8) + l1;
            dyeableTackItem.setColor(itemstack, j2);
            return itemstack;
        }
    }
}